...
class UserProfileForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        # Allow users to keep their own email, only check if it's used by OTHER users
        existing_user = Customer.objects.filter(email=email).exclude(pk=self.instance.pk).first()
        if existing_user:
