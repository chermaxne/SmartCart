    ...
    if request.user.is_authenticated:
        try:
            ...
            if request.user.preferred_category:
                ...
                logger.info(f"ML predicted category for {request.user.username}: {predicted_category}")
                ...
                if predicted_category:
                    try:
                        ...
                        logger.warning(f"Predicted category '{predicted_category}' not found in database")
                        recommended_products = list(Product.objects.filter(
                            is_active=True
                        ).order_by('-rating')[:6])
